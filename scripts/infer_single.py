...
def load_spike(npy_file_path):
    np1 = np.load(npy_file_path)
       
    image_tensor = torch.from_numpy(np1)
    cropped_image = image_tensor[0:250, 0:120, :]
    resized_image = torch.nn.functional.interpolate(cropped_image.unsqueeze(0).float(),
                                                 size=(640, 192), mode='bilinear', align_corners=False)

    np1 = np.array(resized_image, dtype=np.float32).squeeze(0)
    np1[np1 < 0.3] = 0
    np1[np1 >= 0.3] = 1

    #np1 = np.sum(np1[0:128,:,:], axis=0).astype(np.uint8)
    
    return np1
    #print(np1.shape)
    '''
    np1 = Image.fromarray(np1)
    np1 = np1.resize((640, 192), Image.BILINEAR)
    np1 = np1.transpose(Image.FLIP_TOP_BOTTOM)
    '''
...
def infer_and_save_depth(input_file, output_file, model_wrapper, image_shape, half, save):
    ...
    image = load_spike(input_file)

    # Resize and to tensor
    #image = resize_image(image, image_shape)
    image = to_tensor(image).unsqueeze(0)
    #print(image.shape)
    image = torch.sum(image, dim=1, keepdim=True) / 128.0
    
    image = np.array(image).squeeze(0).squeeze(0)
    np1 = Image.fromarray(image)
    np1 = np1.resize((640, 192), Image.BILINEAR)
    np1 = np1.transpose(Image.FLIP_TOP_BOTTOM)  
    image = np.array(np1)
    ...
    pred_inv_depth = model_wrapper.depth_single(image)['inv_depths'][0]
    ...
    if save == 'npz' or save == 'png':
        ...
        #print(rgb.shape, viz_pred_inv_depth.shape)
        ##image = np.concatenate([rgb, viz_pred_inv_depth], 0)
        # Save visualization
        print('Saving {} to {}'.format(
            pcolor(input_file, 'cyan', attrs=['bold']),
            pcolor(output_file, 'magenta', attrs=['bold'])))
        imwrite(output_file, viz_pred_inv_depth) #image[:, :, ::-1])
