...
class S_KITTIDataset(Dataset):
    ...
    def __init__(self, root_dir, file_list, train=True,
        ...
        if self.train:
            self.data_root = "/home/Datadisk/spikedata5622/spiking-2022/SpikeKitti2/train"
        else:
            self.data_root = "/home/Datadisk/spikedata5622/spiking-2022/SpikeKitti2/val"
        
        seq_list = os.listdir(self.data_root)
        for seq in seq_list:
            seq_data_dir = os.path.join(self.data_root, seq, "spike", "r32")
            date = seq[8:10]
            for data in os.listdir(seq_data_dir):
                spike_data_path = os.path.join(seq_data_dir, data)
                #print(date, spike_data_path)
                self.spike_paths.append(spike_data_path)
            if date == "26":
                rgb_seq_path = os.path.join(root_dir, "2011_09_26", seq)
            elif date == "28":
                rgb_seq_path = os.path.join(root_dir, "2011_09_28", seq)
            elif date == "30":
                rgb_seq_path = os.path.join(root_dir, "2011_09_30", seq)
            #print(rgb_seq_path)     
            rgb_seq_dir = os.path.join(rgb_seq_path, "image_03", "data") 
            for rgb_data in os.listdir(rgb_seq_dir):
                
                self.paths.append(os.path.join(rgb_seq_dir, rgb_data))
        
                
                #print(os.path.join(rgb_seq_dir, rgb_data))
        '''
