...
def list2seq(list_, path):
    w = 640 
    h = 192 
        
    mat = np.array(list_).reshape(h, w)
    seq = Image.fromarray(np.uint8(mat))
...
class S_KITTIDataset(Dataset):
    ...
    def __init__(self, root_dir, file_list, train=True,
        ...
        self.spike_root_dir = self.root_dir.replace('raw', 'spike')
        ...
        for i, fname in enumerate(data):
            ...
            #print(seq_name)
            #/home/Datadisk/spikedata5622/spiking-2022/SpikeKitti2/train/
            #/data1/lijianing/Data/KITTI/KITTI_raw/2011_10_03/2011_10_03_drive_0047_sync/image_04/data/spike/r128/xxx.npy
            #/data1/lijianing/Data/KITTI/KITTI_raw/2011_10_03/2011_10_03_drive_0047_sync/image_03/data/xx.png
            if "image_02" in path:
                ##spike_path = path.replace("raw", "spike").replace(".png", ".dat").replace("data/", "").replace("image_02", "image_04")
                spike_path = path.replace(".png", ".npy").replace("image_02", "image_04").replace("data/", "data/spike/r128/")
                #print(spike_path)
            elif "image_03" in path:
                ##spike_path = path.replace("raw", "spike").replace(".png", ".dat").replace("data/", "").replace("image_03", "image_05")
                spike_path = path.replace(".png", ".npy").replace("image_03", "image_05").replace("data/", "data/spike/r128/")
            #print(spike_path)
            if os.path.exists(spike_path):
                self.spike_paths.append(spike_path)
            else:
                pass #print(spike_path)
            add_flag = True
            ...
            if add_flag and self.with_depth:
                ...
                add_flag = depth is not None and os.path.exists(depth)
            if add_flag and "00000.png" not in path:
                self.paths.append(path)
        #print(len(self.spike_paths), len(self.paths))
        # If using context, filter file list
        self.paths.sort(key=lambda x: (x.split("_drive_")[1][:4], x.split(".p")[0][-6:]))
        self.spike_paths.sort(key=lambda x: (x.split("_drive_")[1][:4], x.split(".n")[0][-6:]))
        
        if self.with_context:
            
            paths_with_context = []
            spike_paths_with_context = []
            for stride in strides:
                for idx, file in enumerate(self.paths):
                    ...
                    if idx < 2:
                        self.scene_flag.append(True)
                    else:
                        seq_order_curr = file.split('_drive_')[1][:4]
                        seq_order_past = self.paths[idx-1].split('_drive_')[1][:4]
                        if seq_order_curr == seq_order_past:
                            self.scene_flag.append(False)
                        else:
                            self.scene_flag.append(True)
                    ...
                    if backward_context_idxs is not None and forward_context_idxs is not None:
                        ...
                        spike_paths_with_context.append(self.spike_paths[idx])
                        self.forward_context_paths.append(forward_context_idxs)
                        self.backward_context_paths.append(backward_context_idxs[::-1])
            
            self.paths = paths_with_context[:-4]
            self.spike_paths = spike_paths_with_context[:-4]
        
        if len(self.scene_flag) == 0:
            for idx, spike_s in enumerate(self.spike_paths):
                if idx == 0:
                    self.scene_flag.append(True)
                else:
                    seq_order_past = self.spike_paths[idx-1].split('_drive_')[1][:4]
                    seq_order_curr = spike_s.split('_drive_')[1][:4]
                    if seq_order_curr == seq_order_past:
                        self.scene_flag.append(False)
                    else:
                        self.scene_flag.append(True)                    
        ...
        #print(len(self.scene_flag), len(self.paths), len(self.spike_paths))
        #print(self.spike_paths)
                            
########################################################################################################################     
    #@staticmethod

    @staticmethod
    def analysedat(dat_file_path):
        c = 640
        r = 192
        frame = 0
        frame_tot = 128
        ...
        while(1):
            ...
            if pos >=  r * c: 
                pos = 0                       
                sum[count,:,:] = list2seq(ff, dat_file_path)
                count = count + 1
                ff = []                
        #np.save(dat_file_path.replace(".dat",".npy"), sum)
        #print(np.max(sum[100,:,:]))
        return sum
...
########################################################################################################################
    ...
    def _get_spike_seq(self, spike_file):
        pass
...
########################################################################################################################
    ...
    def _get_spike_sequences(self, idxs):
        """Gets the spike sequences from a .dat file."""
        spike_sequence_path = self.spike_paths[idxs]
        
        #print(self.paths[idxs], self.spike_paths[idxs])
        ##spike_seq = self.analysedat(spike_sequence_path)
        spike_seq = np.load(spike_sequence_path)
        spike_seq = torch.FloatTensor(spike_seq)
        
        return spike_seq

    def _get_scene_flag(self, idxs):
        flag = self.scene_flag[idxs]
        return flag
...
########################################################################################################################
    ...
    def __getitem__(self, idx):
        ...
        sample.update({
            'spike_sequence': self._get_spike_sequences(idx),
        })  
        
        sample.update({
            'seq_flag': self._get_scene_flag(idx),
        })         
