...
from packnet_sfm.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics, average_loss_and_metrics_distill
...
class ModelWrapper(torch.nn.Module):
    ...
    def training_single_step(self, batch, epoch, *args):
        """Processes a training batch."""
        batch = stack_batch(batch)
        output = self.model.forward_single(batch, epoch, progress=self.progress)
        return {
            'loss': output['loss'],
            'metrics': output['metrics']
        }

    def training_step(self, batch, epoch, *args):
        """Processes a training batch."""
        batch = stack_batch(batch)
        output = self.model.forward(batch, epoch, progress=self.progress)
        return {
            'loss': output['loss'],
            'metrics': output['metrics']
        }

    def training_distill_step(self, batch, epoch, *args):
        """Processes a training batch."""
        batch = stack_batch(batch)
        output = self.model.forward_distill(batch, epoch, progress=self.progress)
        return {
            #'loss': output['loss'],
            'loss_distill': output['distill_loss'],
            'metrics': output['metrics']
        }

    def validation_step(self, batch, epoch, *args):
        """Processes a validation batch."""
        output = self.evaluate_depth(batch, epoch)
        if self.logger:
    ...
    def test_step(self, batch, *args):
        ...
        output = self.evaluate_depth(batch, 0)
        #print(1)
    ...
    def training_distill_epoch_end(self, output_batch):
        """Finishes a training epoch."""

        # Calculate and reduce average loss and metrics per GPU
        loss_and_metrics = average_loss_and_metrics_distill(output_batch, 'avg_train')
        loss_and_metrics = reduce_dict(loss_and_metrics, to_item=True)
    ...
    def depth_single(self, *args, **kwargs):
        """Runs the pose network and returns the output."""
        assert self.depth_net is not None, 'Depth network not defined'
        return self.depth_net.forward_single_branch(*args, **kwargs)
    ...
    def evaluate_depth(self, batch, epoch):
        """Evaluate batch to produce depth metrics."""
        # Get predicted depth
        inv_depths = self.model.forward_single(batch, epoch)['inv_depths']  # for spike-only
        #inv_depths = self.model.forward(batch, epoch)['inv_depths']  # for dual-modality                                                   
        ...
        inv_depths_flipped = flip_lr(self.model.forward_single(batch, epoch)['inv_depths'])
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        # Spike_KITTI dataset
        if config.dataset[i] == 'Spike_KITTI':
            from packnet_sfm.datasets.spike_kitti_dataset import S_KITTIDataset
            dataset = S_KITTIDataset(
...
def get_datasampler(dataset, mode):
    ...
    return torch.utils.data.distributed.DistributedSampler(
        dataset, shuffle= False, #(mode=='train'),
