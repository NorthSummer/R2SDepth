...
class Spik_SelfSupModel(SfmModel):
    ...
    def act_ham_loss(self, spike_features, rgb_features):
        ...
        act_loss_func = BinaryHammingDistance(threshold=0.5, multidim_average='global', ignore_index=None, validate_args=True).to(spike_features[0].device)
        all_losses = []
        for spike_feat, rgb_feat in zip(spike_features, rgb_features):
            rgb_feat.detach()
            all_losses.append(act_loss_func(spike_feat, rgb_feat))
        
        return 0.1 * sum(all_losses)    
    

    def relu_list(self, input_s, input_rgb):
        return_list = list()
        for idx, item in enumerate(input_s):
            bin_value = self.relu(item - torch.mean(input_rgb[idx]))
            bin_value[bin_value>0] = 1
            return_list.append(bin_value)
    ...
    def rgb2spike_distillation_loss(self, spike_features, rgb_features):
        ...
        rgb_features_act = self.relu_list(rgb_features, rgb_features)
        ...
        loss2 = self.act_ham_loss(spike_activation_out, rgb_features_act)  
    ...
    def forward(self, batch, epoch, return_logs=False, progress=0.0):
        """
        Processes a batch.
        ...
        output = super().forward(batch, epoch=epoch, return_logs=return_logs)
    ...
    def forward_distill(self, batch, epoch, return_logs=False, progress=0.0):
        """
        Processes a batch.
        ...
        output = super().forward(batch, epoch=epoch-1, return_logs=return_logs)
        ...
        if not self.training:
            ...
            return {
                ...
                **output, **merge_outputs(output, self_sup_output),
            }
            
    def forward_single(self, batch, epoch, return_logs=False, progress=0.0):
        """
        Processes a batch.
        ...
        output_single = super().forward_single(batch, return_logs=return_logs)
        #spike_features = output_spike['spike_encodings']
        #spike_features_out = self.spike_connector_feature(spike_features, 'f')
        ...
        if not self.training:
            ...
            self_sup_output = self.self_supervised_loss(
                batch['rgb_original'], batch['rgb_context_original'],
                output_single['inv_depths'], output_single['poses'], batch['intrinsics'],
                return_logs=return_logs, progress=progress)
            ...
            return {
                'loss': self_sup_output['loss'],
                **merge_outputs(output_single, self_sup_output),
            }